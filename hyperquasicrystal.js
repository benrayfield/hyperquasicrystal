...
//this script only defines the universal function and puts it in the constant hyperquasicrystal.
//Optimizations should go in anyNode.evaler, in another script.
//Common funcs should go in another script.


const hyperquasicrystal = (function(){
	
	TODO put all this stuff in Node.prototype, including ability to change evaler of a Node, t, f, gasTime, gasMem, garbcol, funcallcache.touch, the 12 (or add few more?)
	edge types (which might just be funcallcached not store them in Node), etc.

	Node.prototype.lastId = 1000;
	...
	let Node = function(isLeaf,func,param){
		...
		this.evaler = func.evaler; //optimization
		...
		//just the first 4 params, bitstring size 0-4 bits. cuz opcode is known by then.
		this.o5 = isLeaf ? 1 :  TODO;
	...
	Node.prototype.Node = Node; //cuz wont have the Node var. Have to get it by u().Node
	...
	//the default slow interpreted mode evaler. Can override this by anyLambda().evaler = function(func,param){ ...use GPU.js or js eval or whatever optimization... };
	//TODO make at least 1 evaler just to demonstrate how to do it.
	//
	//eval, call lambda on lambda. TODO gasTime gasFastMem.
	Node.prototype.evaler = function(func,param){
		
		//TODO this.evaler, so can put various optimizations per node. chain of evalers with evaler.on defining which in the chain is used.
		//use the first aNode.evaler where aNode.evaler.on such as aNode.evaler.prev.prev.on.
		
		this.prepay(1,0);
		//let func = this.lam;
		if(this.o7 < 64) return cp(me,z); //less than 7 params. u is a universal lambda of 7 params. at most 6 ever occur in nodes cuz of lazyeval.
		let z = param;
		let y = func.param; //second last param
		let x = func.func.param; //third last param
	...
	let lambdize = node=>{
		...
		let lambda = function(param){
			...
			return NODE.getEvaler()(prepay,NODE.lam,param); //eval lambda call, else throw if not enuf gasTime or gasMem aka prepay(number,number)
		};
		//lambda = lambda.bind(this);
		node.lam = lambda;
	};

	let theUniversalFunction = lambdize(new Node(true,null,null));
	//let op0000001 = u;
	u.func = u(u)(u)(u)(u)(u)(u); //identityFunc.
	u.param = u;
	
	
	theUniversalFunction().gasTime = 1000000;
	theUniversalFunction().gasMem = 1000000;
	theUniversalFunction().prepay(1,2);
	if(theUniversalFunction().gasTime != 999999) throw 'gasTime is broken';
	if(theUniversalFunction().gasMem != 999998) throw 'gasMem is broken';
	
	Node.prototype.getEvaler = function(){
		let evaler = this.evaler;
		while(!evaler.on) evaler = evaler.prev;
		return evaler;
	};
	
	Node.prototype.pushEvaler = function(evaler){
		if(evaler.on === undefined) evaler.on = true;
		evaler.prev = this.evaler;
		this.evaler = evaler;
	};

	return theUniversalFunction;
})();
...
let f = u(u)(u)(u)(u)(u);
f().pushEvaler((prepay,func,param)=>param);

let t = u(u)(u)(u)(uu)(u);
t().pushEvaler((prepay,func,param)=>func.param);

let l = u(u)(u)(uu)(u)(u)(u);
l().pushEvaler((prepay,func,param)=>param.func); //FIXME if wrapping blob, .func and .param are lazyevaled

let r = u(u)(u)(uu)(uu)(u)(u);
r().pushEvaler((prepay,func,param)=>param.param); //FIXME if wrapping blob, .func and .param are lazyevaled

let isLeaf = u(u)(uu)(u)(u)(u)(u);
//FIXME should these near instant evalers that dont allocate heap memory prepay?
r().pushEvaler((prepay,func,param)=>(param==u ? t : f); //FIXME if wrapping blob, .func and .param are lazyevaled
//r().evaler = (func,param)=>(prepay(1,0) || (param==u ? t : f)); //FIXME if wrapping blob, .func and .param are lazyevaled

let s = u(u)(uu)(u)(uu);
s().pushEvaler((prepay,func,z)=>{ prepay(1,0); let x = func.func.param, y = func.param; return x(z)(y(z)); });

let pair = u(u)(uu)(uu)(u);

let typeval = u(u)(uu)(uu)(uu);

let identityFunc = l(u);
identityFunc().pushEvaler((prepay,func,param)=>param); //optimization
...
let isAPair = TODO;
isAPair().pushEvaler((prepay,func,param)=>(param.o5==0b11110 ? t : f));
...
//TODO and().evaler = (func,param)=>( TODO verify its f and t before just verifying they're both t );
